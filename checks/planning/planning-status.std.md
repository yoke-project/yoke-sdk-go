...
| **Planning item** | yoke-project/yoke-sdk-go#5 |

## yoke-sdk-go:planning-status.01 — the workflow runs on the work, and runs nothing of its own
...
## yoke-sdk-go:planning-status.02 — it is granted nothing, and with no credential it does nothing
...
## yoke-sdk-go:planning-status.03 — an item moves forward, and never back
...
## yoke-sdk-go:planning-status.04 — the items are read from the change, and never invented
...
## yoke-sdk-go:planning-status.05 — a query that fails moves nothing, and says so
