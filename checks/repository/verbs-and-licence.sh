...
# std: yoke-sdk-go:verbs-and-licence.01
check_verbs_defined() {
...
# std: yoke-sdk-go:verbs-and-licence.02
...
# std: yoke-sdk-go:verbs-and-licence.03
check_container_build() {
  ...
  "$engine" build -q -t yoke-sdk-go-check-build -f "$root/Containerfile" "$root" >/dev/null \
    || { echo "the image did not build"; return 1; }
  "$engine" run --rm yoke-sdk-go-check-build just build >/dev/null \
    || { echo "build failed inside the container"; return 1; }
}

# std: yoke-sdk-go:verbs-and-licence.04
check_licence() {
...
# std: yoke-sdk-go:verbs-and-licence.05
check_develop_floor() {
  [[ -f "$root/justfile" ]] || { echo "no justfile"; return 1; }
  local out
...
# std: yoke-sdk-go:verbs-and-licence.06
